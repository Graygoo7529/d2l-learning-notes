...
$$
其中 $\eta<1$ 是信任系数，控制单次更新相对于当前权重的大小；$\gamma_t$ 仍是由全局学习率调度器给出的学习率。在不考虑历史梯度的情况下（不使用 momentum 动量），参数更新为：
$$
\mathbf w_{t+1}^{(\ell)}
=
\mathbf w_t^{(\ell)}
...
当 $\|\mathbf w_t^{(\ell)}\|_2$ 和 $\beta$ 固定时，梯度范数 $\|\mathbf g_t^{(\ell)}\|_2$ 越大，分母越大，局部缩放系数 $r_t^{(\ell)}$ 越小；梯度范数越小，$r_t^{(\ell)}$ 越大。因此，LARS 根据梯度相对于权重的大小调整有效学习率：梯度与权重的范数比越大，局部缩放系数越小；该比值越小，局部缩放系数越大，从而协调不同层的相对更新幅度。
...
LARS 最初用于稳定大批量训练：增大批量通常需要提高全局学习率，但不同层能够承受的更新幅度不同，因此需要按层调整有效学习率。当与动量结合使用时，LARS 直接限制本轮梯度与权重衰减项经过缩放后的相对大小（LARS 先按层缩放当前更新，动量再把它与历史更新结合）；不过，由于参数的实际更新还包含历史动量，因此其相对更新量不一定仍以 $\gamma_t\eta$ 为上界。
